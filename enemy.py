...
class Enemy(Tank):
    def __init__(self, img, x, y) -> None:
        Tank.__init__(self, img, x, y)
        self.source_img = pygame.transform.rotate(pygame.transform.scale(img, (40, 70)),180)
        self.image = pygame.transform.rotate(pygame.transform.scale(img, (40, 70)),180)
        self.source_rect = self.source_img.get_rect(x = self.x, y = self.y)
        self.rect = self.image.get_rect(x = self.x, y = self.y)
        self.rotation = 0
        self.combat_range = pygame.Rect(self.rect.x - 200, self.rect.y - 200, 424, 427)
